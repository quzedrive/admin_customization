...
export default function PaymentPolicyPage() {
    ...
    const { data: page, isLoading, error } = usePageBySlug('payment-policy');

    if (isLoading) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center">
                <Loader2 className="w-8 h-8 text-blue-600 animate-spin mb-4" />
                <p className="text-gray-500">Loading Payment Policy...</p>
            </div>
        );
    }
    ...
    if (error || !page) {
        return (
            <div className="min-h-[60vh] flex flex-col items-center justify-center max-w-2xl mx-auto px-4 text-center">
                ...
                <h1 className="text-2xl font-bold text-gray-900 mb-2">Payment Policy Not Found</h1>
                <p className="text-gray-500">
                    The payment policy is currently unavailable or has not been published yet.
                </p>
            </div>
        );
    }
